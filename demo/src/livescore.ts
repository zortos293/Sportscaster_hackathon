import type { CommentaryLine, EventCategory, LiveMatch } from "./apify.js";
...
export async function fetchLiveScoreCommentary(
  matchUrl?: string,
  matchId?: string,
): Promise<{ matchUrl: string; lines: CommentaryLine[]; dataNote?: string; summary?: string }> {
  ...
  if (lines.length === 0) {
    dataNote =
      "LiveScore has no text commentary for this match. Text feeds are usually only available for major tournaments (e.g. World Cup). Try Japan vs Sweden (event ID 1417944) from the dropdown filter.";
  } else if (summary) {
